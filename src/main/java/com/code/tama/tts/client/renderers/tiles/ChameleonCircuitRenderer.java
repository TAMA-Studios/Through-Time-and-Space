...
public class ChameleonCircuitRenderer implements BlockEntityRenderer<ChameleonCircuitPanelTileEntity> {
	...
	@Override
	public void render(@NotNull ChameleonCircuitPanelTileEntity chameleonCircuit, float partialTicks,
		...
		GetTARDISCapSupplier(chameleonCircuit.getLevel()).ifPresent(cap -> {
			...
			fontRenderer.drawInBatch(text, -fontRenderer.width(text) / 2f, 0, 0x55AAFF, true, poseStack.last().pose(),
