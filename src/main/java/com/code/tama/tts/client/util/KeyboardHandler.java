...
public class KeyboardHandler {
  public static final Lazy<KeyMapping> SONIC_MODE =
      Lazy.of(
          () ->
              new KeyMapping(
                  "tts.keybinds.sonic_mode",
                  InputConstants.Type.KEYSYM,
                  InputConstants.KEY_G,
                  "tts.keybinds.catagory"));
    ...
    if (event.phase == TickEvent.Phase.END) {
      while (SONIC_MODE.get().consumeClick()) {
        if (Minecraft.getInstance().player.getMainHandItem().getItem() instanceof SonicItem sonic) {
          ...
          //                            sonic.InteractionType.ordinal() <
          // SonicInteractionType.values().length
          // - 1
