...
/**
 * Coordinate of a single ARS grid cell. One cell is 3x3x3 sections (48x48x48
 * blocks) - the same relationship ChunkPos has to blocks (16), just one size
 * class up and extended to the Y axis as well.
 */
@Getter
public final class ARSPos {
	public static Codec<ARSPos> CODEC = RecordCodecBuilder
			.create(instance -> instance.group(Codec.INT.fieldOf("x").forGetter(ARSPos::getX),
					Codec.INT.fieldOf("y").forGetter(ARSPos::getY), Codec.INT.fieldOf("z").forGetter(ARSPos::getZ))
	...
	public static ARSPos fromBlockPos(BlockPos pos) {
		return new ARSPos(Math.floorDiv(pos.getX(), CELL_SIZE), Math.floorDiv(pos.getY(), CELL_SIZE),
