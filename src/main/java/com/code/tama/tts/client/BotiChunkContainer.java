...
public class BotiChunkContainer {
  ...
  public BotiChunkContainer(
      Level level, BlockState state, FluidState fluidState, BlockPos pos, int light) {
    this.state = state;
  ...
  public static @NotNull BotiChunkContainer decode(@NotNull FriendlyByteBuf buf) {
    ...
    if (IsTile) {
      return new BotiChunkContainer(
          Minecraft.getInstance().level, state, pos, light, true, buf.readNbt());
    }
