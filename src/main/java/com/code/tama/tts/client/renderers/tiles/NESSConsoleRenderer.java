...
public class NESSConsoleRenderer<
        T extends NESSConsoleTile, C extends HierarchicalModel<Entity> & IAnimateableModel<T>>
  ...
  public void render(
    ...
    EMMISIVE =
        new ResourceLocation(MODID, "textures/tiles/console/ness/ness_console_emmisives.png");
    ...
    if (ConsoleTile.getLevel() != null) {
      float offs;
      if (ConsoleTile.getLevel().getBlockState(ConsoleTile.getBlockPos().below()).getBlock()
    ...
    this.MODEL.renderToBuffer(
        poseStack,
        bufferSource.getBuffer(RenderType.entityTranslucent(CONTROLS_ONE)),
    ...
    this.MODEL.renderToBuffer(
        poseStack,
        bufferSource.getBuffer(RenderType.entityTranslucent(CONTROLS_TWO)),
        Minecraft.getInstance().level.getGameTime() % 80 <= 40 ? 0xf000f0 : combinedLight,
        OverlayTexture.NO_OVERLAY,
        1.0f,
        1.0f,
