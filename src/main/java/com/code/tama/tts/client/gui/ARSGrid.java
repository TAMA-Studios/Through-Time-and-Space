...
/**
 * A single ARS room entry: where it sits on the grid, its display name, and the
 * color used to represent it on the map.
 */
@Getter
public class ARSGrid {
	...
	public static ARSGrid deserialize(CompoundTag tag) {
		return new ARSGrid(ARSPos.deserialize(tag.getCompound("Pos")), tag.getString("Name"), tag.getInt("Color"));
	}
}