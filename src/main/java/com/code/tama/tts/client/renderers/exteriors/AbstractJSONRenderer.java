...
public class AbstractJSONRenderer implements IUseJavaJSON, BlockEntityRenderer<ExteriorTile> {
	...
	@Override
	public void render(@NotNull ExteriorTile exteriorTile, float v, PoseStack poseStack,
		...
		if (getModel() != null) {
			getModel().renderToBuffer(poseStack, bufferSource.getBuffer(getRenderType(exteriorTile.Model.getTexture())),
					i, i1, 1, 1, 1, 1); // JavaJSON
		}
		poseStack.popPose();
	}
}
