...
public class CompressedMultiblockRenderer implements BlockEntityRenderer<CompressedMultiblockTile> {
    ...
    public void render(
        ...
        tile.stateMap.forEach((pos, state) -> {
            poseStack.pushPose();
            poseStack.translate(
                    pos.getX() - 0.5, pos.getY() - 0.5, pos.getZ() - 0.5); // Position this mini block relatively
            ...
            int color = blockColors.getColor(
                    state, Minecraft.getInstance().level, pos, 0); // 'level' is the Level, 'pos' is the BlockPos
