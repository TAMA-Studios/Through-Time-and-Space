...
public class CompressedMultiblockRenderer implements BlockEntityRenderer<CompressedMultiblockTile> {
  ...
  @Override
  public void render(
      @NotNull CompressedMultiblockTile tile, // Make sure this is the Tile Entity class
      float partialTicks,
    ...
    poseStack.pushPose();
    poseStack.translate(
        0.5f, 0.5f, 0.5f); // Shift center of structure to origin (accounts for -1 to 2 span)
    poseStack.scale(1 / 3f, 1 / 3f, 1 / 3f); // Shrink to fit inside 1x1x1
    //        poseStack.translate(0.5f, 0.5f, 0.5f);  // Move shrunk structure to block center
    tile.stateMap.forEach(
        (pos, state) -> {
          poseStack.pushPose();
          poseStack.translate(
              pos.getX() - 0.5,
              pos.getY() - 0.5,
              pos.getZ() - 0.5); // Position this mini block relatively
          ...
          int color =
              blockColors.getColor(
                  ...
                  pos,
                  0); // 'level' is the Level, 'pos' is the BlockPos
