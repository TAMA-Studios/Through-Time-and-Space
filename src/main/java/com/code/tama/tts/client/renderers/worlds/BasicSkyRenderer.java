...
public abstract class BasicSkyRenderer extends AbstractLevelRenderer {
  private static void RenderStars(
      @NotNull PoseStack poseStack, Matrix4f projectionMatrix, float PartialTicks) {
    ...
    float size = 0.5f;
    poseStack.translate(
        0, 100, 0); // ((float) relativePosition.x, (float) relativePosition.y, (float)
    // relativePosition.z);
    ...
    for (int i = 0; i < 1000; ++i) {
      ...
      float x =
          190 - new Random(Minecraft.getInstance().level.getGameTime()).nextFloat() * (190 * 2);
      float z =
          190 - new Random(Minecraft.getInstance().level.getGameTime()).nextFloat() * (190 * 2);
      float y = 190;
