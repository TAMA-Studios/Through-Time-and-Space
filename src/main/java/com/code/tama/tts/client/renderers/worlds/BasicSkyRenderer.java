...
public abstract class BasicSkyRenderer extends AbstractLevelRenderer {
	...
	public void RenderLevel(@NotNull Camera camera, Matrix4f matrix4f, @NotNull PoseStack poseStack, Frustum frustum,
			float partialTicks) {
		CustomLevelRenderer.renderImageSky(poseStack, new ResourceLocation(MODID, "textures/environment/night_sky.png"),
				new Vector4i(255, 255, 255, (int) GetOpacityForSkybox(partialTicks)));
		CustomLevelRenderer.renderImageSky(poseStack, new ResourceLocation(MODID, "textures/environment/void.png"),
				new Vector4i(this.SkyColor[0], this.SkyColor[1], this.SkyColor[2],
						-(int) GetOpacityForSkybox(partialTicks)));
		assert Minecraft.getInstance().level != null;
