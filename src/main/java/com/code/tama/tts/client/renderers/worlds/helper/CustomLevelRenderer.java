...
public class CustomLevelRenderer {
  ...
  static long Ticks;
  private static final Vec3 PLANET_POSITION =
      new Vec3(0, 100, 0); // Position of the cube planet in world coordinates
  ...
  public static void applyFogEffect(float ambientLight) {
    float fogDensity =
        Math.max(0.1f, 1.0f - ambientLight); // Inverse relationship with ambient light
  ...
  public static void applyLighting(float ambientLight, boolean Disco) {
    ...
    float rgb[] =
        getCyclingRGB(
            Minecraft.getInstance().level.getGameTime() + Minecraft.getInstance().getPartialTick(),
            0.01f);
  ...
  public static void onRenderLevel(RenderLevelStageEvent event) {
    ...
    if (event.getStage() == RenderLevelStageEvent.Stage.AFTER_SKY) {
      // Get each renderer
      CustomLevelRenderer.Renderers.forEach(
          renderer -> {
  ...
  public static void renderImageSky(
      PoseStack poseStack, ResourceLocation resourceLocation, Vector4i Colors) {
    ...
    bufferBuilder
        ...
        .endVertex();
    poseStack.mulPose(
        Axis.ZN.rotationDegrees(180)); // Restore the matrix so all the other faces aren't affected
  ...
  public static void renderPlanet(
    ...
    RenderSystem.setShaderTexture(
        0, new ResourceLocation(MODID, "textures/environment/" + name + ".png"));
    ...
    poseStack.rotateAround(
        rotation, (float) PivotPoint.x, (float) PivotPoint.y, (float) PivotPoint.z);
