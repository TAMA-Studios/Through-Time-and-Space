...
public abstract class AbstractLevelRenderer {
	...
	public final void Render(Camera camera, Matrix4f matrix4f, PoseStack poseStack, Frustum frustum,
			float partialTicks) {
		// RenderVoid(poseStack);
		assert Minecraft.getInstance().level != null;
		if (!Minecraft.getInstance().level.dimensionType().effectsLocation().equals(this.EffectsLocation()))
			return;

		this.RenderLevel(camera, matrix4f, poseStack, frustum, partialTicks);
		if (this.ShouldRenderVoid())
			renderVoid(poseStack);
	}

	public abstract void RenderLevel(Camera camera, Matrix4f matrix4f, PoseStack poseStack, Frustum frustum,
			float partialTicks);
