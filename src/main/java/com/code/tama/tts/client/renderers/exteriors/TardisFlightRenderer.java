 ...
 * a landed exterior uses, exactly like {@code FallingExteriorRenderer} does for
 * the block-teleport flight mode - except we build the scratch tile from synced
 * entity data instead of an (unsynced) blockData tag, so it actually shows the
 * right model/facing/doors on the client.
 *
 * <p>
 * Known limitation: the scratch tile here is never given a real level/
 * position, so {@code AbstractPortalTile}'s BOTI "window into the interior"
 * won't have anywhere valid to point while flying - you'll get the model, doors
 * and lighting, but not the portal-scene-through-the-windows effect. That's the
 * same limitation the (currently disabled) FallingExteriorEntity render path
 * has.
 * </p>
 */
