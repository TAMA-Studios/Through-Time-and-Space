...
public class ExteriorStatsUICategory extends UICategory {
	...
	public void Render(AbstractMonitorTile monitor, PoseStack poseStack, MultiBufferSource bufferSource,
		...
		GetTARDISCapSupplier(monitor.getLevel()).ifPresent(cap -> {
			...
			fontRenderer.drawInBatch(Component.literal("Exterior Environmental Display").withStyle(style(monitor)),
					-22.5f, 15, color(monitor), false, poseStack.last().pose(), bufferSource, Font.DisplayMode.NORMAL,
					0, combinedLight);
			poseStack.popPose();
