...
public class ExteriorStatsUICategory extends UICategory {
	...
	public void Render(AbstractMonitorTile monitor, PoseStack poseStack, MultiBufferSource bufferSource,
		...
		GetTARDISCapSupplier(monitor.getLevel()).ifPresent(cap -> {
			...
			fontRenderer.drawInBatch(Component.literal("Exterior Environmental").withStyle(style(monitor)), -60f, 29,
					color(monitor), false, poseStack.last().pose(), bufferSource, Font.DisplayMode.NORMAL, 0,
					combinedLight);

			fontRenderer.drawInBatch(Component.literal("Readout").withStyle(style(monitor)), -22.5f, 37, color(monitor),
