...
public class SkyBlock {
  ...
  public static final RenderType SKY_RENDER_TYPE =
      RenderType.create(
          ...
          RenderType.CompositeState.builder()
              ...
              .setTextureState(
                  new RenderStateShard.EmptyTextureStateShard(SkyBlock::setSkyTexture, () -> {}))
  ...
  public static void renderActualSky(Minecraft mc, RenderData renderData) {
    ...
    final boolean hasSpecialFog =
        mc.level.effects().isFoggyAt(Mth.floor(cameraPos.x), Mth.floor(cameraPos.z))
    ...
    levelRenderer.renderSky(
        ...
        () ->
            FogRenderer.setupFog(
                camera, FogRenderer.FogMode.FOG_SKY, renderDistance, hasSpecialFog, delta));
    ...
    if (mc.options.getCloudsType() != CloudStatus.OFF) {
      ...
      levelRenderer.renderClouds(
          poseStack, projectionMatrix, delta, cameraPos.x, cameraPos.y, cameraPos.z);
    }

    RenderSystem.depthMask(false);
    IHelpWithLevelRenderer.TTS$renderSnowAndRain(
        lightTexture, delta, cameraPos.x, cameraPos.y, cameraPos.z);
