...
import static com.code.tama.tts.core.registries.forge.TTSItems.AllValues;
...
public class TTSCreativeTabs {
	...
	public static final RegistryObject<CreativeModeTab> DECORATIONAL_TAB = CREATIVE_MODE_TABS.register(
			"decorational_tab",
			() -> CreativeModeTab.builder().withTabsBefore(TTSCreativeTabs.DIMENSIONAL_TAB.getId())
					...
					.displayItems((parameters, output) -> AllValues().forEach((reg) -> {
						if (AnnotationUtils.hasAnnotation(Decorational.class, reg))
							output.accept(reg.get());
