/* (C) TAMA Studios 2025 */
package com.code.tama.tts.core.worlds.tree.custom;
...
public class GallifreyanOakTrunkPlacer extends TrunkPlacer {
	public static final Codec<GallifreyanOakTrunkPlacer> CODEC = RecordCodecBuilder
			.create((instasnce) -> trunkPlacerParts(instasnce).apply(instasnce, GallifreyanOakTrunkPlacer::new));
	...
	public @NotNull List<FoliagePlacer.FoliageAttachment> placeTrunk(@NotNull LevelSimulatedReader levelSimulatedReader,
			@NotNull BiConsumer<BlockPos, BlockState> blockPosBlockStateBiConsumer, @NotNull RandomSource randomSource,
			int ohLookAtMeImAnInteger, BlockPos pos, @NotNull TreeConfiguration treeConfiguration) {
		...
		for (; j1 >= 0; --j1) {
			...
			if (!(f < 0.0F)) {
				for (int k1 = 0; k1 < l; ++k1) {
					...
					if (this.makeLimb(levelSimulatedReader, blockPosBlockStateBiConsumer, randomSource, blockpos, p1,
							false, treeConfiguration)) {
						...
						if (this.makeLimb(levelSimulatedReader, blockPosBlockStateBiConsumer, randomSource, p2,
								blockpos, false, treeConfiguration)) {
		...
		this.makeLimb(levelSimulatedReader, blockPosBlockStateBiConsumer, randomSource, pos, pos.above(k), true,
				treeConfiguration);
		this.makeBranches(levelSimulatedReader, blockPosBlockStateBiConsumer, randomSource, j, pos, list,
				treeConfiguration);
	...
	private boolean makeLimb(LevelSimulatedReader p_226108_, BiConsumer<BlockPos, BlockState> p_226109_,
			RandomSource p_226110_, BlockPos p_226111_, BlockPos p_226112_, boolean p_226113_,
		...
		if (p_226113_ || !Objects.equals(p_226111_, p_226112_)) {
			...
			for (int j = 0; j <= i; ++j) {
				BlockPos p1 = p_226111_.offset(Mth.floor(0.5F + (float) j * f), Mth.floor(0.5F + (float) j * f1),
						Mth.floor(0.5F + (float) j * f2));
				if (p_226113_) {
					this.placeLog(p_226108_, p_226109_, p_226110_, p1, p_226114_, (p_161826_) -> p_161826_
							.trySetValue(RotatedPillarBlock.AXIS, this.getLogAxis(p_226111_, p1)));
	...
	private void makeBranches(LevelSimulatedReader p_226100_, BiConsumer<BlockPos, BlockState> p_226101_,
			RandomSource p_226102_, int p_226103_, BlockPos p_226104_, List<FoliageCoords> p_226105_,
		...
		for (FoliageCoords fancytrunkplacer$foliagecoords : p_226105_) {
			...
			if (!blockpos.equals(fancytrunkplacer$foliagecoords.attachment.pos())
					&& this.trimBranches(p_226103_, i - p_226104_.getY())) {
				this.makeLimb(p_226100_, p_226101_, p_226102_, blockpos,
						fancytrunkplacer$foliagecoords.attachment.pos(), true, p_226106_);
			}
		}
