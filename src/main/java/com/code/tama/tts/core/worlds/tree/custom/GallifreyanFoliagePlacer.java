...
public class GallifreyanFoliagePlacer extends BlobFoliagePlacer {
	public static final Codec<GallifreyanFoliagePlacer> CODEC = RecordCodecBuilder
			.create((instance) -> blobParts(instance).apply(instance, GallifreyanFoliagePlacer::new));
	...
	protected void createFoliage(@NotNull LevelSimulatedReader levelSimulatedReader,
			FoliagePlacer.@NotNull FoliageSetter foliageSetter, @NotNull RandomSource randomSource,
			@NotNull TreeConfiguration treeConfiguration, int p_225586_,
			FoliagePlacer.@NotNull FoliageAttachment foliageAttachment, int i2, int i3, int i1) {
		for (int i = i1; i >= i1 - i2; --i) {
			int j = i3 + (i != i1 && i != i1 - i2 ? 1 : 0);
			this.placeLeavesRow(levelSimulatedReader, foliageSetter, randomSource, treeConfiguration,
					foliageAttachment.pos(), j, i, foliageAttachment.doubleTrunk());
		}
