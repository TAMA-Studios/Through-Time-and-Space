...
public class DataItemModelProvider extends ItemModelProvider {
	...
	protected void registerModels() {
		...
		simpleBlockItemBlockParent(TTSBlocks.INTERIOR_ROCK);
	...
	private ItemModelBuilder simpleBlockItemBlockParent(RegistryEntry<Block> item) {
		try {
			return withExistingParent("item/" + item.getId().getPath(), modLoc("block/" + item.getId().getPath()));
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
