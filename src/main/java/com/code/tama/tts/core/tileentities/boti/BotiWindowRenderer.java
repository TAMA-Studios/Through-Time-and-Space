...
public class BotiWindowRenderer implements BlockEntityRenderer<BotiWindowTile> {
	...
	public void render(@NotNull BotiWindowTile tile, float partialTick, @NotNull PoseStack pose,
		...
		// The VBO was built in world space, but the pose stack is relative to the tile
		// pos,
		// so we need to undo that offset.
		pose.translate(-masterPos.getX(), -masterPos.getY(), -masterPos.getZ() + 0.5);

		tile.getFBOContainer().Render(pose, (stack, source) -> {
			if (stencilVBO == null)
				return; // I've had instances where it's still null, if this be one of those cases,
						// abort before it crashes
			stack.pushPose();
	...
	/**
	 * Call this when a cluster member is added/removed to force a stencil VBO
	 * rebuild.
	 */
