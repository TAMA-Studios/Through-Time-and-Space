/* (C) TAMA Studios 2025 */
package com.code.tama.tts.core.networking.packets.C2S.entities;
...
public class BlowUpCreeperPacketC2S {
	...
	public static void handle(BlowUpCreeperPacketC2S mes, Supplier<NetworkEvent.Context> context) {
		context.get().enqueueWork(() -> {
			Creeper creeper1 = (Creeper) context.get().getSender().level().getServer()
					.getLevel(context.get().getSender().level().dimension()).getEntity(mes.creeper);
			creeper1.ignite();
		});
