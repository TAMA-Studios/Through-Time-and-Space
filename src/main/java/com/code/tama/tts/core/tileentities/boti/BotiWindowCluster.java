...
public class BotiWindowCluster {
	...
	public static BlockPos electMaster(List<BlockPos> cluster) {
		...
		return cluster.stream().min(Comparator.comparingDouble((BlockPos p) -> distSq(p, fcx, fcy, fcz))
	...
	// Each face is only emitted if the neighboring block is NOT also in the
	// cluster,
	...
	public static VertexBuffer buildStencilVBO(List<BlockPos> cluster) {
		...
		for (BlockPos pos : cluster) {
			...
			for (Direction face : Direction.values()) {
				...
				if (clusterSet.contains(pos.relative(face)))
					continue;
	...
	@OnlyIn(Dist.CLIENT)
	private static void emitFace(BufferBuilder buf, Direction face, float x0, float y0, float z0, float x1, float y1,
			float z1) {
		switch (face) {
			case UP -> {
