...
public class ChooseARSRoomC2S implements ImAPacket {
	...
	public static ChooseARSRoomC2S decode(FriendlyByteBuf buffer) {
		return new ChooseARSRoomC2S(FriendlyByteBufOps.Helper.readWithCodec(buffer, ARSPos.CODEC), buffer.readUtf(), buffer.readInt());
	}
	...
	public static void handle(ChooseARSRoomC2S packet, Supplier<NetworkEvent.Context> contextSupplier) {
		...
		context.enqueueWork(() -> {
			GetTARDISCapSupplier(context.getSender().level()).ifPresent(cap -> {
				WorldHelper.PlaceStructure((ServerLevel) context.getSender().level(), packet.pos.getOrigin().above(ARSRegistry.GetByPath(packet.roomID).HeightOffs()), UniversalCommon.parse(packet.roomID));
