...
public class ControlRegistry {
	...
	public static final ResourceKey<Registry<UICategory>> CONTROL_REGISTRY_KEY = ResourceKey
			.createRegistryKey(new ResourceLocation(MODID, "controls"));

	private static int ID = 0;

	/**
	 * THIS IS ONLY TO BE USED DURING REGISTRATION OF AN ID DO <i>NOT</i> USE IT!
	 * USE {@link ControlRegistry#getMaxID()}}
	 */
	...
	public static void register(IEventBus modEventBus) {
		CONTROLS.makeRegistry(() -> new RegistryBuilder<UICategory>().hasTags().disableSaving().disableSync());
