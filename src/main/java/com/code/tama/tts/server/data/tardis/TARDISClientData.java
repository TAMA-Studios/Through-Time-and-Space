...
public class TARDISClientData {
    ...
    public void setupInteriorDoorPose() {
        this.getExteriorRenderer().getJavaJSON().getPart("IntRightDoor").yRot =
                (float) (TARDIS.GetData().getDoorData().getDoorsOpen() == 2 ? -Math.toRadians(90) : Math.toRadians(0));
        this.getExteriorRenderer().getJavaJSON().getPart("IntLeftDoor").yRot =
                (float) (TARDIS.GetData().getDoorData().getDoorsOpen() >= 1 ? Math.toRadians(90) : Math.toRadians(0));
    }
}
