...
public class SonicItem extends IAmAttunable {
  @Getter @Setter public @NotNull SonicMode InteractionType = new SonicBlockMode();
  ...
  public boolean canAttackBlock(
      @NotNull BlockState p_40962_,
      Level p_40963_,
      @NotNull BlockPos p_40964_,
  ...
  public @NotNull InteractionResultHolder<ItemStack> use(
    ...
    if (interactionHand == InteractionHand.OFF_HAND) {
      if (player.isCrouching()) CycleVariant(player.getOffhandItem());
      else ToggleExtend(player.getOffhandItem());
    ...
    if (player.isCrouching()) {
      for (int i = 0; i < SonicModeRegistry.SONIC_MODE.getEntries().size(); i++) {
        ...
        if (mode.getName().equals(this.InteractionType.getName())) {
          RegistryObject<SonicMode> nextMode =
              SonicModeRegistry.SONIC_MODE.getEntries().stream()
                  .toList()
                  .get(
                      (i + 1 < SonicModeRegistry.SONIC_MODE.getEntries().size() ? i + 1 : 0)
                          % SonicModeRegistry.SONIC_MODE.getEntries().size());
          this.InteractionType = nextMode.get();
          if (!player.level().isClientSide)
            player.sendSystemMessage(
                Component.literal(
                    GrammarNazi.CapitalizeFirstLetters(
                        GrammarNazi.ScoreToSpace(nextMode.get().getName()))));
          break;
        }
      }
    } else {
      if (player.getMainHandItem().getDamageValue()
          >= player.getMainHandItem().getMaxDamage() - 1) {
  ...
  public @NotNull InteractionResult useOn(@NotNull UseOnContext useOnContext) {
    if (useOnContext.getHand() == InteractionHand.OFF_HAND) {
      if (useOnContext.getPlayer().isCrouching()) CycleVariant(useOnContext.getItemInHand());
      else ToggleExtend(useOnContext.getItemInHand());
  ...
  private @NotNull HitResult calculateHitResult(LivingEntity livingEntity) {
    final double MAX_BRUSH_DISTANCE =
        ...
        if (bl) {
          ...
          if (!level.isClientSide()
              && level.getBlockEntity(blockPos)
                  instanceof BrushableBlockEntity brushableBlockEntity) {
            boolean bl2 =
                brushableBlockEntity.brush(
                    level.getGameTime(), player, blockHitResult.getDirection());
            if (bl2) {
              EquipmentSlot equipmentSlot =
                  itemStack.equals(player.getItemBySlot(EquipmentSlot.OFFHAND))
  ...
  public void SetVariant(ItemStack stack, int Variant) {
    ...
    CompoundTag tag = stack.getOrCreateTag();
    if (IsExtended(stack)) Variant *= 100;
    tag.putInt("CustomModelData", Variant);
    stack.setTag(tag);
  }
