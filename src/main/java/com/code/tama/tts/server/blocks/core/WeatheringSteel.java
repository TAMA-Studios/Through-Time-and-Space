...
public interface WeatheringSteel extends ChangeOverTimeBlock<WeatheringSteel.WeatherState> {
    BiMap<RegistryObject<Block>, RegistryObject<Block>> NEXT_BY_BLOCK =
            ImmutableBiMap.<RegistryObject<Block>, RegistryObject<Block>>builder()
                    .put(TTSBlocks.STRUCTURAL_STEEL, TTSBlocks.STRUCTURAL_STEEL_WEATHERED)
                    .put(TTSBlocks.STRUCTURAL_STEEL_WEATHERED, TTSBlocks.STRUCTURAL_STEEL_RUSTED)
                    .build();

    BiMap<RegistryObject<Block>, RegistryObject<Block>> PREVIOUS_BY_BLOCK = NEXT_BY_BLOCK.inverse();

    @Nullable
    private static RegistryObject<Block> RegFromBlock(Block block) {
    ...
    @Nullable
    static Block getPrevious(Block block) {
        if (!prevExists(block)) return null;
    ...
    @Nullable
    static BlockState getPrevious(BlockState state) {
        if (!prevExists(state.getBlock())) return null;
    ...
    @Nullable
    static Block getNext(Block block) {
        if (!nextExists(block)) return null;
