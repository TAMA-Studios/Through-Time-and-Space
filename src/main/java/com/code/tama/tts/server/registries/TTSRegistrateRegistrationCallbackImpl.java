...
public class TTSRegistrateRegistrationCallbackImpl {
	// Intentionally not a synchronized map, since all safe accesses have to be
	// synchronized anyway.
	private static final Map<String, Either<List<CallbackImpl<?, ?>>, TTSRegistrate>> CALLBACKS = new HashMap<>();
			...
			if (either != null) {
				...
				if (optionalCallbacks.isEmpty()) { // in other words, either.right().isPresent()
					throw new IllegalArgumentException(
							"Tried to register a duplicate TTSRegistrate instance for mod ID: " + modid);
				}
	...
	public static <R, T extends R> void register(ResourceKey<? extends Registry<R>> registry, ResourceLocation id,
			NonNullConsumer<? super T> callback) {
		CallbackImpl<R, T> callbackImpl = new CallbackImpl<>(registry, id, callback);

		Either<List<CallbackImpl<?, ?>>, TTSRegistrate> either;
		synchronized (CALLBACKS) {
			either = CALLBACKS.computeIfAbsent(id.getNamespace(), k -> Either.left(new ArrayList<>()));
			// must be synchronized here, because if `registerRegistrate` were called
			// between these two calls,
			// we would be adding to a list that would never be used
			either.ifLeft(callbacks -> callbacks.add(callbackImpl));
		}

		// This is safe to call outside the synchronized block, because a registrate
		// will only ever be added once.
		either.ifRight(callbackImpl::addToRegistrate);
	}

	private record CallbackImpl<R, T extends R>(ResourceKey<? extends Registry<R>> registry, ResourceLocation id,
			NonNullConsumer<? super T> callback) {
		...
		public void addToRegistrate(TTSRegistrate registrate) {
