...
public class SyncTARDISCapPacketS2C {
      ...
      case DataUpdateValues.DATA, DataUpdateValues.RENDERING:
        {
          return new SyncTARDISCapPacketS2C(
              FriendlyByteBufOps.Helper.readWithCodec(buffer, TARDISData.CODEC),
              null,
              null,
              toUpdate);
        }
      case DataUpdateValues.FLIGHT:
        {
          return new SyncTARDISCapPacketS2C(
              null,
              null,
              FriendlyByteBufOps.Helper.readWithCodec(buffer, TARDISFlightData.CODEC),
              toUpdate);
        }
      case DataUpdateValues.NAVIGATIONAL:
        {
          return new SyncTARDISCapPacketS2C(
              null,
              FriendlyByteBufOps.Helper.readWithCodec(buffer, TARDISNavigationalData.CODEC),
              null,
              toUpdate);
        }
      default:
        {
          ...
          TARDISNavigationalData nav =
              FriendlyByteBufOps.Helper.readWithCodec(buffer, TARDISNavigationalData.CODEC);
          TARDISFlightData flight =
              FriendlyByteBufOps.Helper.readWithCodec(buffer, TARDISFlightData.CODEC);
  ...
  public static void encode(SyncTARDISCapPacketS2C packet, FriendlyByteBuf buffer) {
    ...
    switch (packet.toUpdate) {
      ...
      case DataUpdateValues.FLIGHT:
        {
          FriendlyByteBufOps.Helper.writeWithCodec(
              buffer, TARDISFlightData.CODEC, packet.flightData);
          break;
        }
      case DataUpdateValues.NAVIGATIONAL:
        {
          FriendlyByteBufOps.Helper.writeWithCodec(
              buffer, TARDISNavigationalData.CODEC, packet.navigationalData);
          break;
        }
      default:
        {
  ...
  public static void handle(
      SyncTARDISCapPacketS2C packet, Supplier<NetworkEvent.Context> contextSupplier) {
                        ...
                        case DataUpdateValues.DATA, DataUpdateValues.RENDERING:
                          {
                            ...
                            if (cap.GetExteriorTile() != null) {
                              ...
                              cap.GetExteriorTile()
                                  .setModelIndex(cap.GetData().getExteriorModel().getModel());
                            }
                            break;
                          }
                        case DataUpdateValues.FLIGHT:
