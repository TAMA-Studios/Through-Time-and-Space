...
public class SyncTARDISCapPacketS2C {
    public static SyncTARDISCapPacketS2C decode(FriendlyByteBuf buffer) {
        ...
        switch (toUpdate) {
            ...
            case DataUpdateValues.FLIGHT: {
                return new SyncTARDISCapPacketS2C(
                        null, null, FriendlyByteBufOps.Helper.readWithCodec(buffer, TARDISFlightData.CODEC), toUpdate);
            }
                            ...
                            switch (packet.toUpdate) {
                                case DataUpdateValues.DATA, DataUpdateValues.RENDERING: {
                                    ...
                                    if (cap.GetExteriorTile() != null) {
                                        cap.GetExteriorTile().Model =
                                                cap.GetData().getExteriorModel();
                                        cap.GetExteriorTile()
                                                .setModelIndex(cap.GetData()
                                                        .getExteriorModel()
                                                        .getModel());
                                    }
                                    break;
    ...
    public SyncTARDISCapPacketS2C(
            TARDISData data, TARDISNavigationalData navigationalData, TARDISFlightData flightData, int toUpdate) {
        this.data = data;
