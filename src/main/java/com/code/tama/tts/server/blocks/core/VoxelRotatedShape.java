...
public class VoxelRotatedShape {
	...
	public static VoxelShape RotateVoxelShape(VoxelShape shape, Direction dir) {
		...
		for (AABB bb : shape.toAabbs()) {
			newShape = Shapes.join(newShape, Shapes.create(RotateAABB(bb.move(-0.5, 0, -0.5), dir).move(0.5, 0, 0.5)),
					BooleanOp.OR);
		}
	...
	public static AABB RotateAABB(AABB aabb, Direction dir) {
		return new AABB(RotateVec3(new Vec3(aabb.minX, aabb.minY, aabb.minZ), dir),
				RotateVec3(new Vec3(aabb.maxX, aabb.maxY, aabb.maxZ), dir));
	}
