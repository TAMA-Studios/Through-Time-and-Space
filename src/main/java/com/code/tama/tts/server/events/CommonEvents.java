...
public class CommonEvents {
  ...
  public static void onAddReloadListeners(AddReloadListenerEvent event) {
    TTSMod.LOGGER.info(
        "Loaded namespaces: {}", Minecraft.getInstance().getResourceManager().getNamespaces());
        ...
        .ifPresent(
            level -> {
              if (level.GetFlightData().isInFlight()
                  || !level.GetLevel()
                      .players()
                      .isEmpty()) // Only tick if it's in flight or has players in it
              level.Tick();
            });
  }
      ...
      case START:
        {
          ...
          CameraShakeHandler.startShake(
              event.level.GetFlightData().getFlightTerminationProtocol().getTakeoffShakeAmount(),
              999);
          break;
        }
      case END:
      ...
      case END:
        {
          ...
          if (event.level.GetLevel() != null)
            event.level.GetLevel()
                .playSound(
                    null,
                    event.level.GetNavigationalData().GetExteriorLocation().GetBlockPos(),
                    TTSSounds.THUD.get(),
                    SoundSource.BLOCKS);
