...
public class TemporalImprintReaderItem extends PowerableItem {
    BlockPos pos;
    public TemporalImprintReaderItem(Properties properties, int power) {
    ...
    public BlockPos GetLinkedPos(ItemStack stack, Level level) {
        if(pos == null)
            return this.pos = level.getCapability(Capabilities.LEVEL_CAPABILITY).orElseGet(null).GetTIRBlocks().get(stack.getOrCreateTag().getUUID("uuid")).getPos();
