...
public class Capabilities {
  ...
  public static final ResourceLocation FRAGMENT_LINKS =
      new ResourceLocation(MODID, "fragment_links");
  ...
  public static final Capability<IPlayerCap> PLAYER_CAPABILITY =
      CapabilityManager.get(new CapabilityToken<>() {});

  public static <T, O extends ICapabilityProvider> LazyOptional<T> getCap(
      Capability<T> cap, O object) {
  ...
  public static class AttachCapabilities {
    ...
    public static void AttachWorldCapabilities(AttachCapabilitiesEvent<Level> event) {
      ...
      // For unregistered worlds, because some mods either aren't that bright or just as hacky and
      // gimmicky as a
      // nintendo console
      if (event
              .getObject()
      ...
      if (!event
          ...
          .dimensionTypeId()
          .location()
          .equals(MDimensions.TARDIS_DIM_TYPE.location())) return;
    ...
    public static void attachEnergyCapability(AttachCapabilitiesEvent<BlockEntity> event) {
      if (event.getObject() instanceof FragmentLinksTile) {
        ...
        event.addCapability(
            Capabilities.FRAGMENT_LINKS,
            new ICapabilityProvider() {
              @Override
              public <T> @NotNull LazyOptional<T> getCapability(
                  @NotNull Capability<T> cap, Direction side) {
