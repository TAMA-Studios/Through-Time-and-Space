...
public class TBiomeModifiers {
	...
	public static void bootstrap(BootstapContext<BiomeModifier> context) {
		...
		context.register(ADD_ZEITON_ORE,
				new ForgeBiomeModifiers.AddFeaturesBiomeModifier(biomes.getOrThrow(BiomeTags.IS_OVERWORLD),
						HolderSet.direct(placedFeatures.getOrThrow(TPlacedFeatures.ZEITON_ORE_PLACED_KEY)),
						GenerationStep.Decoration.UNDERGROUND_ORES));

		context.register(ADD_NETHER_ZEITON_ORE,
				new ForgeBiomeModifiers.AddFeaturesBiomeModifier(biomes.getOrThrow(BiomeTags.IS_NETHER),
						HolderSet.direct(placedFeatures.getOrThrow(TPlacedFeatures.NETHER_ZEITON_ORE_PLACED_KEY)),
						GenerationStep.Decoration.UNDERGROUND_ORES));

		context.register(ADD_END_ZEITON_ORE,
				new ForgeBiomeModifiers.AddFeaturesBiomeModifier(biomes.getOrThrow(BiomeTags.IS_END),
						HolderSet.direct(placedFeatures.getOrThrow(TPlacedFeatures.END_ZEITON_ORE_PLACED_KEY)),
						GenerationStep.Decoration.UNDERGROUND_ORES));
