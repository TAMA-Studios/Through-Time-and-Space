...
public class TriggerSyncExteriorVariantPacketC2S {
	...
	public static TriggerSyncExteriorVariantPacketC2S decode(FriendlyByteBuf buffer) {
		return new TriggerSyncExteriorVariantPacketC2S(buffer.readResourceKey(Registries.DIMENSION), buffer.readInt(),
	...
	public static void handle(TriggerSyncExteriorVariantPacketC2S packet,
		...
		context.enqueueWork(() -> {
			...
			if (be instanceof ExteriorTile exteriorTile) {
				Networking.sendPacketToDimension(packet.level,
						new SyncExteriorVariantPacketS2C(exteriorTile.getModelIndex(),
								ExteriorsRegistry.GetOrdinal(exteriorTile.GetVariant()), exteriorTile.targetLevel,
								exteriorTile.targetY, exteriorTile.targetPos, packet.blockX, packet.blockY,
								packet.blockZ));
			}
		});
