...
public class FlightSoundSchemesRegistry {
    public static final Codec<AbstractSoundScheme> CODEC =
            Codec.STRING.xmap(FlightSoundSchemesRegistry::GetByName, AbstractSoundScheme::GetName);
    ...
    public static AbstractSoundScheme CycleProt(AbstractSoundScheme structure) {
        for (int i = 0; i < FLIGHT_SOUND_SCHEMES.size() - 1; i++) {
            if (GetSoundScheme(i).equals(structure)) return GetSoundScheme(i + 1);
        }
