...
public class ControlParameters {
  public static Codec<ControlParameters> CODEC =
      RecordCodecBuilder.create(
          instance ->
              instance
                  .group(
                      Codec.FLOAT
                          .fieldOf("helmic_regulator")
                          .forGetter(ControlParameters::getHelmicRegulator),
                      ...
                      Codec.INT
                          .fieldOf("artron_packet_output")
                          .forGetter(ControlParameters::GetArtronPacketOutput),
  ...
  FlightTerminationProtocol flightTerminationProtocol =
      FlightTerminationProtocolRegistry.POLITE_TERMINUS;

  // TODO: Implement Automatic Power Cue
  /**
   * Automatic power cue (APC). The APC function greatly improves the stability and safety of
   * dematerialization and landing. Without the APC, the power drive will switch on and immediately
   * operate at the required energy level for flight, and will shut down just prior to landing. This
   * can result in violent jerks or shudders occurring during a materialization sequence. If the
   * TARDIS is stuck in a tractor beam, a non-APC flight might be able to break the tractor beam a
   * non-APC landing may enable the TARDIS to be snared out of the vortex to an undesirable landing
   * coordinate.
   */
