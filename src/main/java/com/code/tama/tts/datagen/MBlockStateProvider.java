...
public class MBlockStateProvider extends BlockStateProvider {
  ...
  @Override
  public void doorBlockWithRenderType(
      DoorBlock block, ResourceLocation bottom, ResourceLocation top, String renderType) {
    try {
      String baseName = key(block).toString();
      ModelFile bottomLeft =
          models().doorBottomLeft(baseName + "_bottom_left", bottom, top).renderType(renderType);
      ModelFile bottomLeftOpen =
          models()
              .doorBottomLeftOpen(baseName + "_bottom_left_open", bottom, top)
              .renderType(renderType);
      ModelFile bottomRight =
          models().doorBottomRight(baseName + "_bottom_right", bottom, top).renderType(renderType);
      ModelFile bottomRightOpen =
          models()
      ...
      ModelFile topLeftOpen =
          models().doorTopLeftOpen(baseName + "_top_left_open", bottom, top).renderType(renderType);
      ModelFile topRight =
          models().doorTopRight(baseName + "_top_right", bottom, top).renderType(renderType);
      ModelFile topRightOpen =
          models()
              .doorTopRightOpen(baseName + "_top_right_open", bottom, top)
              .renderType(renderType);
      doorBlock(
          block,
          bottomLeft,
  ...
  public void fenceGateBlock(FenceGateBlock block, ResourceLocation texture) {
    try {
      ...
      ModelFile gateWallOpen =
          models().fenceGateWallOpen(key(block).toString() + "_wall_open", texture);
  ...
  @Override
  public void trapdoorBlockWithRenderType(
      TrapDoorBlock block, ResourceLocation texture, boolean orientable, String renderType) {
    try {
      String baseName = key(block).toString();
      ModelFile bottom =
          orientable
              ? models()
                  .trapdoorOrientableBottom(baseName + "_bottom", texture)
                  .renderType(renderType)
  ...
  public void wallBlock(WallBlock block, ResourceLocation texture) {
    try {
      wallBlock(
          block,
          models().wallPost(key(block).toString() + "_post", texture),
