...
public class ModRecipeProvider extends RecipeProvider implements IConditionBuilder {
	private static final List<ItemLike> ZEITON_SMELTABLES = List.of(TTSItems.ZEITON.get(), TTSBlocks.ZEITON_ORE.get(),
			TTSBlocks.DEEPSLATE_ZEITON_ORE.get(), TTSBlocks.NETHER_ZEITON_ORE.get(),
	...
	protected void buildRecipes(Consumer<FinishedRecipe> pWriter) {
		...
		ShapelessRecipeBuilder.shapeless(RecipeCategory.MISC, TTSBlocks.ZEITON_BLOCK.get())
				...
				.unlockedBy(getHasName(TTSItems.ZEITON.get()), has(TTSItems.ZEITON.get())).save(pWriter);

		ShapelessRecipeBuilder.shapeless(RecipeCategory.MISC, TTSItems.ZEITON.get(), 9)
				.requires(TTSBlocks.ZEITON_BLOCK.get())
				.unlockedBy(getHasName(TTSBlocks.ZEITON_BLOCK.get()), has(TTSBlocks.ZEITON_BLOCK.get())).save(pWriter);
	}
}
