...
public class ExteriorItemRenderer extends BlockEntityWithoutLevelRenderer {
    ...
    public void renderByItem(@NotNull ItemStack stack, @NotNull ItemDisplayContext context, @NotNull PoseStack poseStack, @NotNull MultiBufferSource buffer, int packedLight, int packedOverlay) {
        BlockState dummyState = MTMBlocks.EXTERIOR_BLOCK.get().defaultBlockState();
        ExteriorTile dummyBlockEntity = new ExteriorTile(BlockPos.ZERO, dummyState);

        BlockEntityRenderer<ExteriorTile> renderer = Minecraft.getInstance().getBlockEntityRenderDispatcher().getRenderer(dummyBlockEntity);
        poseStack.pushPose();
