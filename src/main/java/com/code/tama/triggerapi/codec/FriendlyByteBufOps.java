...
/**
 * A DynamicOps implementation for serializing/deserializing via FriendlyByteBuf. Supports
...
public class FriendlyByteBufOps implements DynamicOps<FriendlyByteBuf> {
  ...
  public FriendlyByteBuf createList(Stream<FriendlyByteBuf> input) {
    ...
    for (FriendlyByteBuf element : list) {
      // Ensure array() is called on a buffer with a known backing array and no offset.
      // A safer way is using ByteBufUtil.getBytes(element) or element.copy().array()
      // but relying on element.array() for buffers created by 'create*' methods should be fine.
  ...
  @Override
  public DataResult<FriendlyByteBuf> mergeToMap(
      FriendlyByteBuf map, FriendlyByteBuf key, FriendlyByteBuf value) {
    ...
    try {
      ...
      if (ml != null) {
        ...
        ml.entries()
            .forEach(p -> m.put(new FriendlyByteBuf(p.getFirst().copy()).readUtf(), p.getSecond()));
      }
  ...
  @Override
  public DataResult<FriendlyByteBuf> mergeToMap(
      FriendlyByteBuf map, MapLike<FriendlyByteBuf> values) {
    // NOTE: Must duplicate 'map' before calling getMap, as getMap consumes it.
    try {
      FriendlyByteBuf safeMap = new FriendlyByteBuf(map.copy());
      MapLike<FriendlyByteBuf> ml = getMap(safeMap).result().orElse(null);
  ...
  @Override
  public DataResult<Stream<Pair<FriendlyByteBuf, FriendlyByteBuf>>> getMapValues(
  ...
  public FriendlyByteBuf remove(FriendlyByteBuf input, String key) {
    ...
    // Must duplicate key buffers from the MapLike entries before consuming them!
    Map<String, FriendlyByteBuf> m =
        ml.entries()
            .collect(
                Collectors.toMap(
                    p -> new FriendlyByteBuf(p.getFirst().copy()).readUtf(), Pair::getSecond));
    m.remove(key);
  ...
  public static class Helper {
    ...
    public static <T> void writeWithCodec(FriendlyByteBuf buf, Codec<T> codec, T value) {
      FriendlyByteBuf temp =
          codec
              .encodeStart(FriendlyByteBufOps.INSTANCE, value)
    ...
    public static <T> T readWithCodec(FriendlyByteBuf buf, Codec<T> codec) {
      ...
      return codec
          .parse(FriendlyByteBufOps.INSTANCE, temp)
