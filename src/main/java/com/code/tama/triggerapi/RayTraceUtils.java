...
public class RayTraceUtils {
  public static EntityHitResult rayTraceEntity(Entity source, double maxDistance) {
    ...
    return source
        ...
        .getEntities(
            source,
            source.getBoundingBox().expandTowards(look.scale(maxDistance)).inflate(1.0),
            e -> true)
        .stream()
        .map(
            e ->
                new EntityHitResult(
                    e, start.add(look.scale(start.distanceTo(e.getPosition(1.0F))))))
        .filter(hit -> hit.getEntity().isPickable())
        .min(
            (a, b) ->
                (int) (start.distanceToSqr(a.getLocation()) - start.distanceToSqr(b.getLocation())))
        .orElse(null);
  }
  ...
  public static HitResult rayTraceFromEntity(Entity entity, double maxDistance, boolean hitFluids) {
    ...
    Vec3 endPos =
        eyePos.add(lookVec.x * maxDistance, lookVec.y * maxDistance, lookVec.z * maxDistance);
    return entity
        .level()
        .clip(
            new ClipContext(
