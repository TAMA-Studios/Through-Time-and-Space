...
public class RayTraceUtils {
    public static HitResult rayTraceFromEntity(Entity entity, double maxDistance, boolean hitFluids) {
        ...
        return entity.level().clip(new ClipContext(eyePos, endPos, ClipContext.Block.OUTLINE, hitFluids ? ClipContext.Fluid.ANY : ClipContext.Fluid.NONE, entity));
    }
    ...
    public static EntityHitResult rayTraceEntity(Entity source, double maxDistance) {
        ...
        return source.level().getEntities(source, source.getBoundingBox().expandTowards(look.scale(maxDistance)).inflate(1.0), e -> true)
                .stream()
