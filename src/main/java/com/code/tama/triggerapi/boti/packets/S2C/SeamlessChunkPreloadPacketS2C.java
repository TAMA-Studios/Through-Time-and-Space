...
/**
 * Carries a batch of pre-gathered destination geometry for a seamless
 * teleport.  Unlike {@link PortalChunkDataPacketS2C} (which targets a portal tile
 * entity for BOTI rendering), this packet targets
 * {@link ClientSeamlessTeleportState}'s staging buffer.
 *
 * Multiple packets are sent (one per batch of <= maxBlocks containers).
 * Each packet carries:
 *  - the teleport UUID     (so stale packets from a prior teleport are dropped)
 *  - index / total         (so the client knows when all batches have arrived)
 *  - the container batch
 *
...
public class SeamlessChunkPreloadPacketS2C implements ImAPacket {
    ...
    public SeamlessChunkPreloadPacketS2C(UUID teleportId,
                                         List<BotiBlockContainer> containers,
                                         int index,
                                         int total) {
    ...
    public static void handle(SeamlessChunkPreloadPacketS2C msg, Supplier<NetworkEvent.Context> ctxSupplier) {
        ...
        ctx.enqueueWork(() ->
            DistExecutor.unsafeRunWhenOn(Dist.CLIENT, () -> () ->
                ClientSeamlessTeleportState.stageContainers(
                    msg.teleportId,
                    msg.containers,
                    msg.index,
                    msg.total)
