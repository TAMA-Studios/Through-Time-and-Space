 ...
 *     completion the gathered batches are broadcast to the portal's dimension
 *     via {@link PortalChunkDataPacketS2C}.
 *
 ...
 * THREAD SAFETY
 * -------------
 * The thread submits small work items to the main server thread via
 * {@code server.submit(...).join()} where Minecraft requires it (chunk loading,
 * light engine access), and does everything else off-thread.
 */
...
public class ChunkGatheringThread extends Thread {
    ...
    @Nullable
    private final AbstractPortalTile portalTile;

    /**
     * Non-null in TELEPORT mode.
     * Receives (batchList, totalBatchCount) once per batch.
     */
    @Nullable
    private final BiConsumer<List<BotiBlockContainer>, Integer> resultCallback;
    ...
    @Deprecated
    public ChunkGatheringThread(int chunks,
                                ServerLevel level,
     ...
     *
     * @param chunks          number of chunks per axis to gather
     * @param sourceLevel     the level the portal tile lives in (used only for
     *                        its server reference; geometry is gathered from
     *                        {@code destLevel})
     * @param destLevel       the destination level whose geometry to gather
     * @param targetPos       centre of the region to gather
     * @param yaw             portal facing yaw (for behind-portal culling)
     * @param resultCallback  receives each batch; first arg is the batch,
     *                        second arg is the TOTAL number of batches that
     *                        will be sent (known only after gathering; the
     *                        callback is invoked once per batch with the same
    ...
    public ChunkGatheringThread(int chunks,
                                ...
                                BlockPos targetPos,
                                float yaw,
                                BiConsumer<List<BotiBlockContainer>, Integer> resultCallback) {
        this.setName("BOTIChunkGatheringThread");
        this.chunks         = chunks;
    ...
    public void run() {
        ...
        try {
            ...
            int chunksToRender = Math.min(this.chunks,
                ...
                for (int v = vMin + 1; v < vMax; v++) {
                    ...
                    if (!chunk.isLightCorrect()) {
                        targetLevel.getServer().submit(() -> {
                            for (int ly = targetLevel.getMinBuildHeight();
                                 ly < targetLevel.getMaxBuildHeight(); ly += 16) {
                                targetLevel.getLightEngine().checkBlock(new BlockPos(
                        ...
                        for (int y = 0; y < 16; y++) {
                            for (int x = 0; x < 16; x++) {
                                for (int z = 0; z < 16; z++) {
                                    ...
                                    DataLayer bl = targetLevel.getLightEngine()
                                            .getLayerListener(LightLayer.BLOCK)
                                            .getDataLayerData(SectionPos.of(samplePos));
                                    DataLayer sl = targetLevel.getLightEngine()
                                    ...
                                    DataLayer blA = targetLevel.getLightEngine()
                                            .getLayerListener(LightLayer.BLOCK)
                                            .getDataLayerData(SectionPos.of(samplePosA));
                                    DataLayer slA = targetLevel.getLightEngine()
                                            .getLayerListener(LightLayer.SKY)
                                            .getDataLayerData(SectionPos.of(samplePosA));
                ...
                for (int ly = 0; ly < sizeY; ly++) {
                    for (int lz = 0; lz < sizeZ; lz++) {
                        ...
                        for (int d = 0; d < 6; d++) {
                            ...
                            if (reachable[nx][ny][nz]) { exposed = true; break; }
                        }
                        if (!exposed) continue;
                ...
                for (int i = 0; i < batches.size(); i++) {
                    ...
                    Networking.INSTANCE.send(
                            ...
                            new PortalChunkDataPacketS2C(
                                    portalTile.getBlockPos(), batches.get(idx), idx, total));
                }
            }
