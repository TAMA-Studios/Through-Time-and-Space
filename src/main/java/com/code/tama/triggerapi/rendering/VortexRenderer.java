...
public class VortexRenderer {
  ...
  public VortexRenderer(ResourceLocation texture) {
    ...
    SECOND_LAYER_LOCATION =
            texture.withPath(texture.getPath().replace(".png", "") + "_two" + ".png");
    THIRD_LAYER_LOCATION =
            texture.withPath(texture.getPath().replace(".png", "") + "_three" + ".png");
  ...
  public void renderVortexLayer(PoseStack stack, float scaleFactor) {
    ResourceLocation currentTexture =
        scaleFactor == 1.5f ? SECOND_LAYER_LOCATION : THIRD_LAYER_LOCATION;
  ...
  private float computeDistortionFactor(float time, int t) {
    return (float)
            (Math.sin(
                    time * this.wobbleSpeed * 2.0 * Math.PI
                        + (13 - t) * this.wobbleSeperationFactor)
                * this.wobbleFactor)
        / 8;
  }
}
