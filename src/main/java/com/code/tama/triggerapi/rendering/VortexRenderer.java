...
public class VortexRenderer {
    ...
    public VortexRenderer(ResourceLocation baseTexture) {
        ...
        textureLayers.put(LayerType.SECOND, baseTexture.withPath(baseTexture.getPath().replace(".png", "") + "_two.png"));
        textureLayers.put(LayerType.THIRD, baseTexture.withPath(baseTexture.getPath().replace(".png", "") + "_three.png"));

        // Pre-check resources once; store null if missing to avoid expensive per-frame checks
    ...
    private void renderSection(VertexConsumer builder, int zOffset, LevelRenderContext ctx) {
        ...
        for (int uOffset = 0; uOffset < 6; uOffset++) {
            ...
            switch (uOffset) {
                case 0 -> addQuad(builder, ctx,
                        0f, -startScale + distortion, -zOffset,
                        0f, -endScale + distortionPlusOne, -zOffset - 1,
                        -endScale * SQRT3_OVER_2, endScale / -2f + distortionPlusOne, -zOffset - 1,
                        -startScale * SQRT3_OVER_2, startScale / -2f + distortion, -zOffset,
                        uPanelOffset, uPanelOffsetPlus, v1, v2);
                case 1 -> addQuad(builder, ctx,
                        -startScale * SQRT3_OVER_2, startScale / -2f + distortion, -zOffset,
                        -endScale * SQRT3_OVER_2, endScale / -2f + distortionPlusOne, -zOffset - 1,
                        -endScale * SQRT3_OVER_2, endScale / 2f + distortionPlusOne, -zOffset - 1,
                        -startScale * SQRT3_OVER_2, startScale / 2f + distortion, -zOffset,
                        uPanelOffset, uPanelOffsetPlus, v1, v2);
                case 2 -> addQuad(builder, ctx,
                        0f, endScale + distortionPlusOne, -zOffset - 1,
                        0f, startScale + distortion, -zOffset,
                        -startScale * SQRT3_OVER_2, startScale / 2f + distortion, -zOffset,
                        -endScale * SQRT3_OVER_2, endScale / 2f + distortionPlusOne, -zOffset - 1,
                        uPanelOffset, uPanelOffsetPlus, v1, v2);
                case 3 -> addQuad(builder, ctx,
                        0f, startScale + distortion, -zOffset,
                        0f, endScale + distortionPlusOne, -zOffset - 1,
                        endScale * SQRT3_OVER_2, endScale / 2f + distortionPlusOne, -zOffset - 1,
                        startScale * SQRT3_OVER_2, startScale / 2f + distortion, -zOffset,
                        uPanelOffset, uPanelOffsetPlus, v1, v2);
                case 4 -> addQuad(builder, ctx,
                        startScale * SQRT3_OVER_2, startScale / 2f + distortion, -zOffset,
                        endScale * SQRT3_OVER_2, endScale / 2f + distortionPlusOne, -zOffset - 1,
                        endScale * SQRT3_OVER_2, endScale / -2f + distortionPlusOne, -zOffset - 1,
                        startScale * SQRT3_OVER_2, startScale / -2f + distortion, -zOffset,
                        uPanelOffset, uPanelOffsetPlus, v1, v2);
                case 5 -> addQuad(builder, ctx,
                        0f, -endScale + distortionPlusOne, -zOffset - 1,
                        0f, -startScale + distortion, -zOffset,
                        startScale * SQRT3_OVER_2, startScale / -2f + distortion, -zOffset,
                        endScale * SQRT3_OVER_2, endScale / -2f + distortionPlusOne, -zOffset - 1,
    ...
    private void addQuad(VertexConsumer builder, LevelRenderContext ctx,
    ...
    private void addVertex(VertexConsumer builder, LevelRenderContext ctx,
                           float x, float y, float z, float u, float v) {
        builder.vertex(ctx.pose, x, y, z)
                .color(1f, 1f, 1f, 1f)
                .uv(u, v)
                .uv2(FULL_BRIGHT)
                .normal(ctx.normal, 0f, 0f, 0f)
                .endVertex();
    }
    ...
    private float computeDistortionFactor(float time, int t) {
        return (float) (Math.sin(time * wobbleSpeed * 2.0 * Math.PI + (13 - t) * wobbleSeparation)
                * wobbleAmplitude) / 8f;
    }

    public enum LayerType { BASE, SECOND, THIRD }
