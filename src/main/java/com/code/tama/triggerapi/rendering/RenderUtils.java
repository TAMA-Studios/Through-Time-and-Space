 ...
 *
 * The pattern here is always: Java handles MC objects and GL calls, Rust
 * handles bulk math on flat float arrays. Never call the native methods for
 * single operations, the JNI overhead isn't worth it. Use them when you have
 * 10+ objects to process per frame.
 */
public class RenderUtils {
	 ...
	 *
	 * @param aabbs
	 *            flat array of [minX,minY,minZ,maxX,maxY,maxZ, ...], one entry per
	 *            AABB
	 * @param frustumPlanes
	 *            6 planes × 4 floats [nx,ny,nz,d] = 24 floats total
	...
	/**
	 * Generates all 24 line-segment vertices (bottom face, top face, 4 vertical
	 * edges) for N AABBs at once. Output per vertex: [x, y, z, nx, ny, nz], 144
	 * floats per AABB. Feed the result directly into a VertexConsumer; no
	 * per-vertex math needed in Java.
	 *
	 * @param aabbs
	 *            flat [minX,minY,minZ,maxX,maxY,maxZ, ...]
	 */
	 ...
	 *            flat [minX,minY,minZ,maxX,maxY,maxZ, ...]
	 * @param reverse
	 *            1 = farthest first (use for transparent geometry), 0 = nearest
	 *            first
	...
	/**
	 * Transforms N xyz points by a 4×4 column-major matrix (same layout as
	 * JOML/GL).
	 *
	 * @param points
	 *            flat [x,y,z, x,y,z, ...]
	...
	/**
	 * Generates vertices for a wireframe sphere as line segment pairs. Returns flat
	 * [x,y,z, x,y,z, ...], each consecutive pair is one line.
	 *
	 * @param stacks
	 *            latitude resolution (8–16 is plenty for debug rendering)
	 * @param slices
	 *            longitude resolution
	 */
	public static native float[] buildSphereVertices(float cx, float cy, float cz, float radius, int stacks,
			int slices);

	/**
	 * Computes normalised direction vectors for a batch of line segments. Useful
	 * when you need to supply normals for RenderType.lines() separately.
	 *
	 * @param lines
	 ...
	 *            flat [minX,minY,minZ,maxX,maxY,maxZ, ...]
	 * @param amount
	 *            how much to expand on each side (negative = shrink)
	 */
	...
	/**
	 * Packs a list of AABBs into the flat float array format Rust expects. Call
	 * once, reuse the result across frustumCull / buildAABBVertices /
	 * sortByDistance.
	 */
	public static float[] packAABBs(AABB... boxes) {
		...
		for (int i = 0; i < boxes.length; i++) {
			AABB b = boxes[i];
			int base = i * 6;
			out[base] = (float) b.minX;
			out[base + 1] = (float) b.minY;
			out[base + 2] = (float) b.minZ;
			out[base + 3] = (float) b.maxX;
			out[base + 4] = (float) b.maxY;
			out[base + 5] = (float) b.maxZ;
		}
		return out;
	}
	 ...
	 *
	 * @param vertexData
	 *            output of buildAABBVertices()
	 * @param pose
	 *            current PoseStack.last()
	 * @param consumer
	 *            the VertexConsumer for RenderType.lines()
	 * @param r,g,b,a
	 *            line colour
	 */
	public static void submitAABBVertices(float[] vertexData, PoseStack.Pose pose, VertexConsumer consumer, float r,
			float g, float b, float a) {

		// Each vertex is 6 floats: [x, y, z, nx, ny, nz]
		for (int i = 0; i < vertexData.length; i += 6) {
			consumer.vertex(pose.pose(), vertexData[i], vertexData[i + 1], vertexData[i + 2]).color(r, g, b, a)
					.normal(pose.normal(), vertexData[i + 3], vertexData[i + 4], vertexData[i + 5]).endVertex();
		}
	}
	 ...
	 */
	public static void submitSphereVertices(float[] vertexData, PoseStack.Pose pose, VertexConsumer consumer, float r,
			float g, float b, float a) {
		...
		for (int i = 0, n = 0; i < vertexData.length; i += 6, n += 3) {
			...
			consumer.vertex(pose.pose(), vertexData[i + 3], vertexData[i + 4], vertexData[i + 5]).color(r, g, b, a)
					.normal(pose.normal(), nx, ny, nz).endVertex();
		}
	}
	...
	// // Pack all AABBs once
	// AABB[] boxes =
	// entities.stream().map(Entity::getBoundingBox).toArray(AABB[]::new);
	...
	// // int[] sorted = RenderUtils.sortByDistance(culledPacked, camX, camY, camZ,
	// 1);
	//
	...
	// RenderUtils.submitAABBVertices(verts, poseStack.last(), consumer, r, g, b,
	// a);
