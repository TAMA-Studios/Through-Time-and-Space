...
public class JavaJSONModel extends Model {
    ...
    public void renderToBuffer(
        ...
        if (model != null && alpha > 0) {
            ...
            // Normal Textures
            renderType =
                    JavaJSONRenderer.transparentRenderType(model.getModelInfo().getTexture());
            renderLayer(
                    poseStack, bufferSource.getBuffer(renderType), packedLight, packedOverlay, red, green, blue, alpha);

            // Light Map
            if (model.getModelInfo().getLightMap() != null) {
                renderType =
                        JavaJSONRenderer.lightMapRenderType(model.getModelInfo().getLightMap());
                renderLayer(
                        poseStack,
                        bufferSource.getBuffer(renderType),
                        0xf000f0, // Max light
