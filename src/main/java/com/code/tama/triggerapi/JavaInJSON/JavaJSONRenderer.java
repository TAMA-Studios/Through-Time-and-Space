  ...
  protected static ResourceLocation generateAlphaOverlay(ResourceLocation texture) {
    try {
      ...
      NativeImage originalImage =
          NativeImage.read(mc.getResourceManager().getResourceOrThrow(texture).open());
      NativeImage newImage =
          new NativeImage(originalImage.getWidth(), originalImage.getHeight(), false);
      ...
      ResourceLocation registered =
          textureManager.register(newLocation.getPath(), new DynamicTexture(newImage));
      return registered;
    } catch (IOException e) {
