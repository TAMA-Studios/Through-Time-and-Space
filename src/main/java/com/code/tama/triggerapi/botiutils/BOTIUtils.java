...
public class BOTIUtils {
    ...
    for (Direction dir : directions) {
      ...
      if (neighborContainer != null) {
        if (BOTIUtils.shouldRenderFace(
            state,
            neighborContainer.getState(),
            Minecraft.getInstance().level,
            pos,
            dir,
            neighbourPos)) quads.addAll(model.getQuads(state, dir, rand));
      } else quads.addAll(model.getQuads(state, dir, rand));
    }
    return quads;
  }

  public static Map<BlockPos, BotiChunkContainer> getMapFromContainerList(
      List<BotiChunkContainer> list) {
  ...
  public static VertexBuffer buildModelVBO(
      List<BotiChunkContainer> containers, AbstractPortalTile tile) {
  ...
  public static void updateChunkModel(AbstractPortalTile tileEntity) {
    ...
    if (tileEntity.targetLevel != null)
      Networking.INSTANCE.sendToServer(
          new PortalChunkRequestPacketC2S(
  ...
  public static boolean shouldRenderFace(
    ...
    if (state.skipRendering(neighbor, dir)) {
      ...
      Block.BlockStatePairKey block$blockstatepairkey =
          new Block.BlockStatePairKey(state, neighbor, dir);
      ...
      if (b0 != 127) {
        ...
        if (voxelshape.isEmpty()) {
          ...
          VoxelShape voxelshape1 =
              neighbor.getFaceOcclusionShape(level, secondPos, dir.getOpposite());
          boolean flag = Shapes.joinIsNotEmpty(voxelshape, voxelshape1, BooleanOp.ONLY_FIRST);
          if (object2bytelinkedopenhashmap.size() == 2048) {
            object2bytelinkedopenhashmap.removeLastByte();
          }

          object2bytelinkedopenhashmap.putAndMoveToFirst(
              block$blockstatepairkey, (byte) (flag ? 1 : 0));
          return flag;
        }
      }
    } else {
  ...
  public static void RenderScene(PoseStack pose, AbstractPortalTile portal) {
    ...
    if (portal.MODEL_VBO == null) { // It'll be null the first time it's accessed, forcing a build
      portal.MODEL_VBO =
          BOTIUtils.buildModelVBO(portal.containers, portal); // Build VBO so it's not null
      ...
      minecraft
          ...
          .ifPresent(
              cap -> {
                ...
                pose.mulPose(
                    Axis.YP.rotationDegrees(cap.GetNavigationalData().getFacing().toYRot()));
              });
